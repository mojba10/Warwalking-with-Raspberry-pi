...
- **Wi-Fi scanning** — SSID, BSSID, channel, signal, encryption type, via `nmcli`.
- **GPS tagging** — every scan is linked to the coordinates where it happened.
- **Security scoring** — combines encryption strength, default/vendor SSID patterns, and (optionally) a dictionary attack with `aircrack-ng` into a single risk score.
...
core/
  ...
  security_analyzer.py   # risk scoring + optional password audit
  evil_twin_detector.py  # evil-twin detection logic
database/
  db_manager.py          # SQLite storage (AP + scan history tables)
web/
  app.py                 # Flask backend
  templates/index.html   # map + table frontend (Leaflet.js)
```
